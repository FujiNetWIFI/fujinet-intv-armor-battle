...
if ! grep -q '127\.0\.0\.1' "$BUILD/srv_endpoint.asm" 2>/dev/null; then
    ...
    echo "  make SRV_HOST=127.0.0.1 build/armor_net1.bin build/armor_net2.bin"
    exit 1
fi
...
FN2=$!
python3 server/intv_relay_server.py --port 9104 > "$RIG/server.log" 2>&1 &
SRV=$!
